...
func (controller *panelController) defineControlsHandlers() (err error) {
	...
	*/
	...
	if controller.keyWPM = document.ElementByID("keyWPM"); controller.keyWPM == nil {
		err = errors.New("unable to find #keyWPM")
		return
	}
	// Handle the keyWPM button's onchange event.
	controller.keyWPM.SetEventHandler(controller.handleOnChange, "change", false)
...
func (controller *panelController) handleOnChange(e event.Event) (nilReturn interface{}) {
	controller.record.WPM, _ = controller.keyWPM.ValueUint64()
	controller.messenger.updateKeyWPM(controller.record)
	return
}
