...
func NewPanel(quitChan, eojChan chan struct{}, receiveChan lpc.Receiving, sendChan lpc.Sending, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("KeyWPMPanel: %w", err)
		}
	}()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: document.ElementByID("mainMasterView_home_pad_KeyButton_KeyReadyPanel_tab_bar-KeyWPMTabPanel-H3"),
		tabButton:      document.ElementByID("mainMasterView_home_pad_KeyButton_KeyReadyPanel_tab_bar-KeyWPMTab"),
	}
	messenger := &panelMessenger{
		group: group,
	}
