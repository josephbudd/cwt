...
func NewPanel(ctx context.Context, ctxCancel context.CancelFunc, receiveChan lpc.Receiving, sendChan lpc.Sending, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("KeyTestPanel: %w", err)
		}
	}()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: document.ElementByID("mainMasterView_home_pad_KeyButton_KeyReadyPanel_tab_bar-KeyTestTabPanel-H3"),
		tabButton:      document.ElementByID("mainMasterView_home_pad_KeyButton_KeyReadyPanel_tab_bar-KeyTestTab"),
	}
	messenger := &panelMessenger{
		group: group,
	}
