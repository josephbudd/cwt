...
func (messenger *panelMessenger) getKeyCodesWPMCB(msg *message.GetTextToKeyMainProcessToRenderer) {
	...
	messenger.controller.keyWidget.SetKeyCodesHelpWPM(msg.Solution, msg.Help, msg.WPM)
}
...
func (messenger *panelMessenger) checkUserKeyCB(msg *message.CheckKeyMainProcessToRenderer) {
	if msg.State&state != state {
		return
	}
	if msg.Error {
			...
			case msg := <-receiveCh:
				...
				*/
				...
				case *message.GetTextToKeyMainProcessToRenderer:
					messenger.getKeyCodesWPMCB(msg)
				case *message.CheckKeyMainProcessToRenderer:
					messenger.checkUserKeyCB(msg)
