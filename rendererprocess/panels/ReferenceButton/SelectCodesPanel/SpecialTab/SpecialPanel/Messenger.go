...
func (messenger *panelMessenger) updateKeyCodeCB(msg *message.UpdateKeyCodeMainProcessToRenderer) {
	...
	if msg.Record.Selected {
		display.Success(fmt.Sprintf("The %q is now selected.", msg.Record.Name))
	} else {
		display.Success(fmt.Sprintf("The %q is no longer selected.", msg.Record.Name))
	}
}
...
func (messenger *panelMessenger) getKeyCodesCB(msg *message.GetKeyCodesMainProcessToRenderer) {
	...
	messenger.controller.setup(getSpecialKeyCodes(msg.Records))
}

func getSpecialKeyCodes(records []*record.KeyCode) (group []*record.KeyCode) {
	group = make([]*record.KeyCode, 0, len(records))
	for _, r := range records {
		if r.Type == data.KeyCodeTypeSpecial {
