...
func NewPanel(ctx context.Context, ctxCancel context.CancelFunc, receiveChan lpc.Receiving, sendChan lpc.Sending, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("SpecialPanel: %w", err)
		}
	}()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: document.ElementByID("mainMasterView_home_pad_ReferenceButton_SelectCodesPanel_tab_bar-SpecialTabPanel-H3"),
		tabButton:      document.ElementByID("mainMasterView_home_pad_ReferenceButton_SelectCodesPanel_tab_bar-SpecialTab"),
	}
	messenger := &panelMessenger{
		group: group,
	}
