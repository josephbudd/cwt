...
func NewPanel(quitChan, eojChan chan struct{}, receiveChan lpc.Receiving, sendChan lpc.Sending, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("PunctuationPanel: %w", err)
		}
	}()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: document.ElementByID("mainMasterView_home_pad_ReferenceButton_SelectCodesPanel_tab_bar-PunctuationTabPanel-H3"),
		tabButton:      document.ElementByID("mainMasterView_home_pad_ReferenceButton_SelectCodesPanel_tab_bar-PunctuationTab"),
	}
	messenger := &panelMessenger{
		group: group,
	}
