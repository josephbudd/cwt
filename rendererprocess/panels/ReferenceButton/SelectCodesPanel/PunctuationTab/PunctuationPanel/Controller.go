...
func (controller *panelController) setup(rr []*record.KeyCode) {
	...
	for _, r := range rr {
		...
		checkbox := document.ElementByID(checkboxID)
		checkbox.SetEventHandler(controller.handleChecked, "change", false)
	}
}

func (controller *panelController) handleChecked(e event.Event) (nilReturn interface{}) {
	target := document.NewElementFromJSValue(e.JSTarget)
	target.Blur()
	id, _ := target.AttributeUint64(recordIDAttribute)
	r := controller.records[id]
	r.Selected = target.Checked()
	controller.messenger.updateKeyCode(r)
	return
}
