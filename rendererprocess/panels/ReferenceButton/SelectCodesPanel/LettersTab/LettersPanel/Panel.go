...
func NewPanel(ctx context.Context, ctxCancel context.CancelFunc, receiveChan lpc.Receiving, sendChan lpc.Sending, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("LettersPanel: %w", err)
		}
	}()
	...
	state = help.StateLetter()
	testState = help.StateTest()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: document.ElementByID("mainMasterView_home_pad_ReferenceButton_SelectCodesPanel_tab_bar-LettersTabPanel-H3"),
		tabButton:      document.ElementByID("mainMasterView_home_pad_ReferenceButton_SelectCodesPanel_tab_bar-LettersTab"),
	}
	messenger := &panelMessenger{
		group: group,
	}
