...
func NewPanel(quitChan, eojChan chan struct{}, receiveChan lpc.Receiving, sendChan lpc.Sending, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("CopyPracticePanel: %w", err)
		}
	}()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: document.ElementByID("mainMasterView_home_pad_CopyButton_CopyReadyPanel_tab_bar-CopyPracticeTabPanel-H3"),
		tabButton:      document.ElementByID("mainMasterView_home_pad_CopyButton_CopyReadyPanel_tab_bar-CopyPracticeTab"),
	}
	messenger := &panelMessenger{
		group: group,
	}
