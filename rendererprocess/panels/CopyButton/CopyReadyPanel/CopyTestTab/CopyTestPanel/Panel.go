...
func NewPanel(ctx context.Context, ctxCancel context.CancelFunc, receiveChan lpc.Receiving, sendChan lpc.Sending, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = fmt.Errorf("CopyTestPanel: %w", err)
		}
	}()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: document.ElementByID("mainMasterView_home_pad_CopyButton_CopyReadyPanel_tab_bar-CopyTestTabPanel-H3"),
		tabButton:      document.ElementByID("mainMasterView_home_pad_CopyButton_CopyReadyPanel_tab_bar-CopyTestTab"),
	}
	messenger := &panelMessenger{
		group: group,
	}
