...
// CreateElementLabelForID invokes the document's createElement.
func (notjs *NotJS) CreateElementLabelForID(text, forID string) js.Value {
	label := notjs.document.Call(createElementMethodName, "label")
	tn := notjs.CreateTextNode(text)
	notjs.AppendChild(label, tn)
	notjs.SetFor(label, forID)
	return label
}
