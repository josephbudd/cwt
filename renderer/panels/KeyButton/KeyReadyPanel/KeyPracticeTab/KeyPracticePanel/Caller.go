...
func (caller *panelCaller) getKeyCodesWPMCB(msg *message.GetTextToKeyMainProcessToRenderer) {
	...
	caller.controller.keyWidget.SetKeyCodesHelpWPM(msg.Solution, msg.Help, msg.WPM)
}
...
func (caller *panelCaller) checkUserKeyCB(msg *message.CheckKeyMainProcessToRenderer) {
	if msg.State&state != state {
		return
	}
	if msg.Error {
			...
			case msg := <-receiveCh:
				...
				*/
				...
				case *message.GetTextToKeyMainProcessToRenderer:
					caller.getKeyCodesWPMCB(msg)
				case *message.CheckKeyMainProcessToRenderer:
					caller.checkUserKeyCB(msg)
