...
func NewPanel(quitChan, eojChan chan struct{}, receiveChan lpc.Receiving, sendChan lpc.Sending, vtools *viewtools.Tools, njs *notjs.NotJS, help *paneling.Help) (panel *Panel, err error) {
	...
	defer func() {
		if err != nil {
			err = errors.WithMessage(err, "SpecialPanel")
		}
	}()
	...
	presenter := &panelPresenter{
		group:          group,
		tabPanelHeader: notJS.GetElementByID("tabsMasterView_home_pad_ReferenceButton_SelectCodesPanel_tab_bar-SpecialTabPanel-H3"),
	}
	caller := &panelCaller{
		group: group,
	}
