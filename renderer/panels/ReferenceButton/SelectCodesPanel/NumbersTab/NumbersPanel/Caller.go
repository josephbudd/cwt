...
func (caller *panelCaller) updateKeyCodeCB(msg *message.UpdateKeyCodeMainProcessToRenderer) {
	...
	if msg.Record.Selected {
		tools.Success(fmt.Sprintf("The number %q is now selected.", msg.Record.Name))
	} else {
		tools.Success(fmt.Sprintf("The number %q is no longer selected.", msg.Record.Name))
	}
}
...
func (caller *panelCaller) getKeyCodesCB(msg *message.GetKeyCodesMainProcessToRenderer) {
	...
	caller.controller.setup(getNumberKeyCodes(msg.Records))
}
