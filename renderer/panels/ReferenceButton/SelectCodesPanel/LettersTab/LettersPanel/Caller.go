...
func (caller *panelCaller) updateKeyCode(record *record.KeyCode) {
...
func (caller *panelCaller) updateKeyCodeCB(msg *message.UpdateKeyCodeMainProcessToRenderer) {
	...
	if msg.Record.Selected {
		tools.Success(fmt.Sprintf("The letter %q is now selected.", msg.Record.Name))
	} else {
		tools.Success(fmt.Sprintf("The letter %q is no longer selected.", msg.Record.Name))
	}
}

// GetKeyCodes

func (caller *panelCaller) getKeyCodes() {
...
func (caller *panelCaller) getKeyCodesCB(msg *message.GetKeyCodesMainProcessToRenderer) {
	...
	caller.controller.setup(getLetterKeyCodes(msg.Records))
}
