	...
	case *types.MainProcessToRendererUpdateKeyCodeCallParams:
		if params.State&panelCaller.state == panelCaller.state {
			...
			if params.Record.Selected {
				panelCaller.tools.Success(fmt.Sprintf("The punctuation %q is now selected.", params.Record.Name))
			} else {
				panelCaller.tools.Success(fmt.Sprintf("The punctuation %q is no longer selected.", params.Record.Name))
			}
		}
	}
}
...
func (panelCaller *Caller) getKeyCodesCB(params interface{}) {
	...
	case *types.MainProcessToRendererGetKeyCodesCallParams:
		...
		// no errors
		panelCaller.controler.setup(getPunctuationKeyCodes(params.Records))
	default:
		panelCaller.tools.Error("getKeyCodesCB: Unknown type.")
	}
}

func getPunctuationKeyCodes(records []*types.KeyCodeRecord) []*types.KeyCodeRecord {
	group := make([]*types.KeyCodeRecord, 0, len(records))
	for _, r := range records {
		if r.Type == keyCodeTypes.KeyCodeTypePunctuation {
