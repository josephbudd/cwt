...
func (controller *panelController) defineControlsReceiveEvents() (err error) {
	...
	*/
	...
	if controller.copyTestCopy = notJS.GetElementByID("copyTestCopy"); controller.copyTestCopy == null {
		err = errors.New("unable to find #copyTestCopy")
		return
	}

	// Define the start button.
	if controller.copyTestStart = notJS.GetElementByID("copyTestStart"); controller.copyTestStart == null {
		err = errors.New("unable to find #copyTestStart")
		return
	}
	// Receive the start button's onclick event.
	controller.receiveEvent(controller.copyTestStart, "onclick", false, false, false)

	// Define the check button.
	if controller.copyTestCheck = notJS.GetElementByID("copyTestCheck"); controller.copyTestCheck == null {
		err = errors.New("unable to find #copyTestCheck")
		return
	}
	// Receive the check button's onclick event.
	controller.receiveEvent(controller.copyTestCheck, "onclick", false, false, false)
...
func (controller *panelController) handleCheck(event js.Value) {
	...
	copy := strings.TrimSpace(notJS.GetValue(controller.copyTestCopy))
	if len(copy) == 0 {
		...
		for {
			...
			case event = <-controller.eventCh:
				...
				*/
				...
				case controller.copyTestCheck:
					controller.handleCheck(event.Event)
				case controller.copyTestStart:
