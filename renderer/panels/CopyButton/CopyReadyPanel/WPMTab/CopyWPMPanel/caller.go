...
func (panelCaller *Caller) updateCopyWPM(record *types.WPMRecord) {
	params := &types.RendererToMainProcessUpdateCopyWPMCallParams{
		Record: record,
	}
	panelCaller.updateCopyWPMConnection.CallMainProcess(params)
}
...
func (panelCaller *Caller) updateCopyWPMCB(params interface{}) {
	...
	case *types.MainProcessToRendererUpdateCopyWPMCallParams:
		...
		panelCaller.tools.Success("Copy WPM Updated.")
	}
}
...
func (panelCaller *Caller) initialCalls() {
	...
	*/
	...
	panelCaller.getCopyWPMConnection.CallMainProcess(nil)
}
